...
- **A self-hosted NGINX reverse proxy**, running on the GCP showcase
  VM, the one piece of infrastructure genuinely outside both Azure
...
The brief names both as equally valid. I checked every realistic
managed alternative directly against its own current pricing before
choosing, rather than assuming the free route by default:
...
its own. A self-hosted NGINX proxy does the same job, checking a
backend's health and forwarding to whichever is up, for zero cost, and
it's literally the other option the brief names, not a fallback.
...
real requests fail, and the backup receives zero real requests while
the primary is healthy. If both regions failed at the same instant, the
...
described in the concept docs rather than built. One more live moving
part isn't worth the risk for a limitation this narrow.
...
*Above: Uptime Kuma watching all three addresses independently, before
anything is touched.*

![Uptime Kuma independently catching West Europe going down](docs/screenshots/73-kuma-west-europe-down-alert.png)
*Above: West Europe's own monitor going down, checked directly rather
...
*Above: same address, `https://136.115.185.153/`, different region. The
user experienced nothing; the platform rerouted itself.*
...
*Above: nothing manually pointed back at West Europe. NGINX's own
health check retried it on schedule and switched back automatically.*
...
debugging session of the build.** Two separate problems stacked on top
of each other:
...
lesson, check `ps aux | grep ansible-playbook` before every retry.
...
90. Azure only accepts priorities from 100 to 4096. Fixed by moving it
to 105, still evaluated ahead of the existing allow-rule at 120, just
...
*Above: left in on purpose rather than cropped out. A wrong value
caught and fixed against a real API response is more honest than a
script that only ever ran once.*
